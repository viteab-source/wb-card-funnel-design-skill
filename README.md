# WB Card Funnel Design Skill

Навык для исследования конкурентов Wildberries, проектирования контентной воронки и подготовки проверяемой инфографики карточки товара.

## Что умеет

- проверяет точный SKU и подтверждённые характеристики;
- показывает shortlist конкурентов и ждёт явного одобрения пользователя;
- разбирает полную галерею каждого одобренного конкурента;
- строит неповторяющуюся последовательность кадров по вопросам покупателя;
- блокирует неподтверждённые claims, подмену товара и неполный research-gate;
- проверяет медиа, OCR и готовность выдачи локальными Python-валидаторами.

## Самая простая установка через Codex

Отправьте агенту ссылку на этот репозиторий и текст из [`wb-card-funnel-design/INSTALL-PROMPT.md`](wb-card-funnel-design/INSTALL-PROMPT.md). Codex с системным `skill-installer` скачает каталог скилла, а затем автоматически подготовит локальную среду. После установки скилл доступен на следующем ходе.

## Автоматическая установка из клона

```bash
git clone https://github.com/viteab-source/wb-card-funnel-design-skill.git
cd wb-card-funnel-design-skill
python3 install.py
```

На Windows используйте `py -3 install.py` или `python install.py`. Установщик сам копирует скилл в Codex, создаёт приватную `.runtime`, ставит Pillow и запускает doctor. Права администратора не нужны.

## Требования

- Codex с доступом к локальным файлам;
- Python 3.10+; остальные локальные зависимости ставятся автоматически в каталог скилла;
- браузер или другой разрешённый источник для исследования актуальных карточек;
- генератор изображений или графический редактор, если нужны готовые визуалы;
- `MPSTATS_TOKEN` опционален и читается только из окружения.

Кириллический Inter и OFL-лицензия входят в пакет. Локальный рендерер одинаково собирает точный текст, плашки и пользовательские изображения на macOS и Windows. Доступ к `imagegen` и web/browser определяется аккаунтом и приложением Codex и не может быть установлен GitHub-репозиторием.

## Проверка

```bash
python3 -m unittest discover -s wb-card-funnel-design/scripts -p 'test_*.py'
python3 /path/to/skill-creator/scripts/quick_validate.py wb-card-funnel-design
```

Публичный CI повторяет установку, doctor, полный набор тестов и smoke-render на macOS и Windows с Python 3.11 и 3.13.

## Обновление

После выхода новой версии человек пишет Codex: «обнови `wb-card-funnel-design`». Агент сначала проверяет релиз, а по прямой команде применяет его:

```bash
python3 ~/.codex/skills/wb-card-funnel-design/scripts/update_skill.py --check
python3 ~/.codex/skills/wb-card-funnel-design/scripts/update_skill.py --apply
```

На Windows используется `py -3` или `python`. Обновлятор скачивает последний GitHub Release, проверяет версию, полностью тестирует новую копию и только затем заменяет установленный каталог, сохраняя предыдущую версию как backup. Пользовательские проекты, артефакты и API-ключи в каталог скилла не записываются и обновлением не затрагиваются.

API-ключи не входят в репозиторий. При появлении интеграций каждый пользователь хранит собственный ключ в переменной окружения или защищённом хранилище Codex.

## Ограничения публичного пакета

Пакет не содержит фотографии конкурентов, пользовательские исходники, токены, рабочие артефакты и внутреннюю библиотеку вкуса. Они создаются локально у каждого тестировщика. Навык не публикует карточки и не меняет данные Wildberries.

## Обратная связь тестировщика

Укажите товар, входные материалы, найденный shortlist, решение по конкурентам, получившуюся воронку, ошибки валидаторов и 2–3 скриншота результата. Не прикладывайте ключи, cookies и выгрузки кабинета с чувствительными данными.

## Лицензия

До выбора постоянной лицензии разрешено использовать этот release candidate только для тестирования и обратной связи.
